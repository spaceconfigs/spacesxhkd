...
if [ $# -lt 1 ]; then
  echo "Usage: $0 \"title\" [--workspace] [--date] [--cpu] [--ram] [--disk] [--volume] [--all]"
  exit 1
fi
...
OPTIONS=$(getopt -o "" \
    --long workspace,date,cpu,ram,disk,volume,all \
    -- "$@")
...
cpu_temperature=$(sensors | grep 'Package id' | awk '{print $4}')
...
if [ "$show_cpu_temperature" = true ]; then
  message+="CPU temperature: ${cpu_temperature}%\n"
fi
